...
test("the shared selector exposes exactly three neutral options only after setup and education", () => {
...
test("reload chooses splash, dashboard, the next session gate, or the option selector and never restores working state", () => {
  ...
  assert.equal(afterSetup.screen, "edu", "device completion resumes at required education");
  assert.equal(afterSetup.playKey, null, "education restoration is silent");
...
test("Back renders only on the established screens and targets what it targets", () => {
  ...
  assert.deepEqual(shell.backTarget({ ...st, screen: "setup", setupSeen: true }), { kind: "screen", screen: "ear" });
  assert.deepEqual(shell.backTarget({ ...st, screen: "edu", setupSeen: true }), { kind: "screen", screen: "setup" });
  assert.deepEqual(shell.backTarget({ ...st, screen: "setup", setupSeen: true, eduSeen: true }), { kind: "screen", screen: "home" });
  assert.deepEqual(shell.backTarget({ ...st, screen: "edu", setupSeen: true, eduSeen: true }), { kind: "screen", screen: "home" });
  assert.deepEqual(shell.backTarget({ ...st, screen: "edu", eduSeen: true }), { kind: "screen", screen: "setup" });
